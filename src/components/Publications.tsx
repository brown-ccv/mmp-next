            ...
            return (
              <div key={i}>
                  {publication.tags && (publication.tags.map((tag) => {
                      return (<span key={tag}
                                    className={tag === "MMP" ? "bg-secondary-brown-500 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded-2xl" : "bg-secondary-blue-700 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded-2xl"}
                      >{tag}</span>)
                  }))
